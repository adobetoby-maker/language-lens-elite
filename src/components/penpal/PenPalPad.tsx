...
import { Mail, ChevronRight, CheckCircle2, XCircle, RefreshCw, Trophy, PenLine, BookOpen, Sparkles } from "lucide-react";
...
function VocabCard({ text, translation, pronunciation }: { text: string; translation: string; pronunciation?: string }) {
  return (
    <div className="rounded-xl border border-border/50 bg-card/50 px-4 py-3 text-left">
      ...
      {pronunciation && (
        <p className="font-mono text-[10px] text-gold mb-1">{pronunciation}</p>
      )}
...
function CircleQuiz({ question, choices, correct, onResult }: {
  question: string;
  choices: string[];
  correct: number;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {choices.map((c, i) => {
          ...
          if (selected !== null && !isSelected && isCorrect) style = "border-green-500/40 bg-green-500/5 text-green-400/60";
          return (
            <button
              key={i}
              onClick={() => pick(i)}
            ...
            >
              {/* Circle indicator */}
              <span className={`absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full border-2 text-[9px] transition-all ${
                isSelected
                  ? isCorrect
...
function CheckResult({ correct, recognized, feedback, onNext }: {
  correct: boolean;
  recognized: string;
  feedback: string;
  onNext: () => void;
  ...
  return (
    <div className={`rounded-2xl border p-5 ${correct ? "border-green-500/30 bg-green-500/5" : "border-amber-500/30 bg-amber-500/5"}`}>
      ...
      {recognized && (
        <p className="text-xs text-muted-foreground mb-1">AI read: <em>"{recognized}"</em></p>
      )}
...
export function PenPalPad() {
  ...
  const [checkResult, setCheckResult] = useState<{ correct: boolean; recognized: string; feedback: string } | null>(null);
  const [checking, setChecking] = useState(false);
  const [completedTopics, setCompletedTopics] = useState<Set<string>>(new Set());
  const [circleCorrect, setCircleCorrect] = useState<boolean | null>(null);

  const topic: PenPalTopic | null = topicIndex !== null && curriculum
    ? curriculum.topics[topicIndex]
    : null;

  const allDone = curriculum
    ? completedTopics.size >= curriculum.topics.length
    : false;

  const handleCheck = async (imageBase64: string, targetText: string, mode: "word" | "sentence" | "free") => {
    setChecking(true);
    try {
  ...
  const advance = () => {
    ...
    if (step === "vocab") setStep("copyWord");
      ...
      if (checkResult?.recognized && state.userVocab.length > 0 && state.vocabLang === state.selectedLanguage) {
        const recognized = checkResult.recognized.toLowerCase();
        const usedVocabWord = state.userVocab.some((v) => recognized.includes(v.word.toLowerCase()));
    ...
    return (
      <div className="fade-in mx-auto flex max-w-2xl flex-col items-center justify-center gap-4 py-24 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-sm">
          Pen Pal Practice is coming soon for <strong>{state.selectedLanguage}</strong>.
          Switch to Spanish, French, Japanese, Korean, German, Portuguese, or Italian to get started.
        </p>
      </div>
    );
  }
      ...
      <div className="fade-in mx-auto flex max-w-2xl flex-col items-center justify-center gap-6 py-20 text-center">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm max-w-sm">
            You've written a full self-introduction letter to {curriculum.penpalName} in {curriculum.penpalCity}.
            Your brain just fired a lot of neurons it won't forget.
          </p>
        </div>
        ...
        <button
          onClick={() => { setCompletedTopics(new Set()); setTopicIndex(null); dispatch({ type: "ADD_XP", payload: 30 }); }}
    ...
    return (
      <div className="fade-in mx-auto max-w-2xl space-y-6 py-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Write a letter to {curriculum.penpalName} in {curriculum.penpalCity}. One topic at a time.
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-gold/20 bg-gold/[0.03] px-5 py-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-gold mb-2">Your letter starts with</p>
          <p className="font-display text-lg text-foreground">{curriculum.greeting} <span className="text-muted-foreground">___…</span></p>
        </div>
          ...
          return (
            <div key={phase} className="space-y-2">
              <div className="flex items-center justify-between px-1">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold">{phaseLabel}</p>
                ...
                {phaseTopics.map(({ t, i }) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ borderColor: done ? "rgba(74,222,128,0.3)" : undefined, backgroundColor: done ? "rgba(74,222,128,0.04)" : undefined }}
                    >
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold shrink-0 ${done ? "bg-green-500/20 text-green-400" : "bg-border/30 text-muted-foreground"}`}>
                        {done ? "✓" : i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm">{t.topic}</p>
                        <p className="text-xs text-muted-foreground truncate">{t.question} — {t.questionEnglish}</p>
                      </div>
                      <ChevronRight className="h-4 w-4 text-muted-foreground/40 shrink-0" />
                    </button>
                  );
  ...
  return (
    <div className="fade-in mx-auto max-w-2xl space-y-6 py-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <button onClick={backToTopics} className="text-xs text-muted-foreground hover:text-foreground transition-colors mb-1">
            ← Back to topics
          </button>
          <h2 className="font-display text-xl font-semibold">{topic.topic}</h2>
          <p className="text-sm text-muted-foreground">{topic.question} — <em>{topic.questionEnglish}</em></p>
        </div>
      </div>
      ...
      <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-gold">{stepLabels[step]}</p>

      {/* ── Step: Vocab ── */}
      {step === "vocab" && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Learn these words. You'll write them next.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {topic.vocab.map((v, i) => <VocabCard key={i} {...v} />)}
          </div>
          <button
            onClick={advance}
      ...
      {step === "circle" && (
        <div className="space-y-4">
          ...
          />
          {circleCorrect !== null && (
            <div className={`rounded-xl border px-4 py-3 text-sm ${circleCorrect ? "border-green-500/30 bg-green-500/5 text-green-400" : "border-amber-500/30 bg-amber-500/5 text-amber-400"}`}>
              {circleCorrect ? "✓ Correct! Your brain just made a connection." : "Not quite — but you've seen it now. You won't forget it."}
              <button onClick={advance} className="ml-3 underline underline-offset-2 opacity-70 hover:opacity-100">Continue →</button>
            </div>
          )}
        </div>
      )}
          ...
          {(() => {
            ...
            return (
              <div className="rounded-xl border border-violet-500/20 bg-violet-500/[0.04] px-4 py-3">
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-violet-400 mb-2">Pattern hints</p>
                <div className="flex flex-col gap-1.5">
                  {ultraPatterns.map((p) => (
                    <div key={p.id} className="flex items-baseline gap-2">
                      <span className="font-mono text-[10px] text-violet-300/70 w-24 shrink-0">{p.name}</span>
