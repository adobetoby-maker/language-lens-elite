...
export const translatePhrases = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ phrases: TranslatedPhrase[] | null; error: string | null }> => {
...
${data.phrases.map((p, i) => `${i + 1}. ${p}`).join("\n")}`,
      ...
      if (!toolUse || toolUse.type !== "tool_use") return { phrases: null, error: "Translation failed" };
